...
export function UpcomingHighlight() {
        ...
        if (data?.data?.events && data.data.events.length > 0) {
          const transformed = data.data.events.map((apiEvent: any) => {
            const totalQuantitySold =
              apiEvent.latest_addition?.tickets?.reduce(
            ...
            return {
              ...
              image:
                apiEvent.image ||
                "https://images.unsplash.com/photo-1519703936-c4a3b3eb88e4?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  ...
  return (
    <Box id="upcoming" py={80} style={{ backgroundColor: "#f8f9fa" }}>
      <Container size="lg">
        {/* Section Header */}
        <Group
